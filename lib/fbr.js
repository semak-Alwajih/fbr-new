...
export function calcTotals(lines, scenarioCode) {
  ...
  const normalized = lines.map(line => ({
    ...
    taxRate: Number(line.taxRate || scenario.taxRate || 0),
    hsCode: line.hsCode || '',
    uom: line.uom || '',
    saleType: line.saleType || scenario.saleType || '',
...
export function buildFbrPayload({ invoice, customer, settings }) {
  ...
  return {
    ...
    items: invoice.lines.map((line) => {
      ...
      const salesTaxApplicable = valueSalesExcludingST * Number(line.taxRate || scenario.taxRate || 0) / 100
      const furtherTax = valueSalesExcludingST * Number(scenario.furtherTax || 0) / 100
      return {
        hsCode: line.hsCode || '',
        productDescription: line.description || '',
        rate: `${Number(line.taxRate || scenario.taxRate || 0)}%`,
        uoM: line.uom || '',
        quantity: Number(line.quantity || 0),
...
export function validateBeforeFbr({ invoice, customer, settings }) {
  ...
  if (!seller) errors.push('Seller NTN/CNIC is required')
  if (!(seller.length === 7 || seller.length === 13)) errors.push('Seller NTN/CNIC must be 7-digit NTN or 13-digit CNIC')
